...
static void setGLMatrix(GLenum type, const QMatrix4x4& m)
{
    ...
    if (sizeof(qreal) == sizeof(GLfloat)) {
        ...
        GLfloat mat[16];
        const qreal *data = (double*)m.constData();
...
void BezierWidget::allocVertices()
{
    ...
    if (num > numVertices) {
        positions = (QVector4D *)std::realloc(positions, num * sizeof(GLfloat) * 4);
        texCoords = (QVector2D *)std::realloc(texCoords, num * sizeof(GLfloat) * 2);

        int maxIndices = num * 6;
        indices = (IndexType *)std::realloc(indices, maxIndices * sizeof(IndexType));
    }
    numVertices = num;
#ifdef USE_VBOS
